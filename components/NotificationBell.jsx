    ...
    const updateNotifications = async () => {
      try {
        ...
        if (localStorage.getItem(NOTIFICATION_STORAGE_KEY)) {
          const pnt = JSON.parse(
            localStorage.getItem(NOTIFICATION_STORAGE_KEY)
          ).notifications;
          const newNotification = !nt.every((notification) =>
            pnt.map((notification) => notification.id).includes(notification.id)
          );
          const oldDot =
  ...
  return (
    <>
      <IconButton
        ...
        onClick={handleClick}
      >
        <Badge color="primary" variant={dot ? 'dot' : null}>
          <NotificationsIcon />
        </Badge>
      </IconButton>
      <Menu
      ...
      >
        {notifications.length ? (
          ...
          <Box sx={{ m: 1, px: 2 }}>
            <Typography variant="p" sx={{ m: 0, fontSize: '1rem' }}>
