...
interface IProcessPaymentProps {
  ...
  onTransactionSuccess: (txId: string, senderAddress: string) => void;
}
...
const ProcessPayment: FC<IProcessPaymentProps> = ({
      ...
      if (payment && payment.length > 0 && submitting === undefined) {
        ...
        try {
          if (paymentWalletType === 'nautilus') {
            ...
            if (connected) {
              ...
              if (tx) {
                ...
                onTransactionSuccess(ok, changeAddress)
          ...
          if (paymentWalletType === 'mobile') {
            const init = await initMobileTx.mutateAsync({ payment: JSON.stringify(payment), recipientAddress })
  ...
  useEffect(() => {
    if (pollScan.data) {
      setTransactionId(pollScan.data.txId)
      setSenderAddress(pollScan.data.address)
      setScanned(true);
  ...
  useEffect(() => {
    if (pollComplete.data !== null && pollComplete.data >= 0) {
      ...
      onTransactionSuccess(transactionId, senderAddress)
