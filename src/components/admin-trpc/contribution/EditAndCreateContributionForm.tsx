...
type EditAndCreateContributionFormProps = {
  mode: 'create' | 'edit';
  selectedRound?: string | null;  // Only for 'edit' mode
...
const EditAndCreateContributionForm: FC<EditAndCreateContributionFormProps> = ({ mode, project, selectedRound }) => {
  ...
  useEffect(() => {
    const sluggedProjectName = projectSlugify(project?.name)
    setProjectSlug(sluggedProjectName)
  }, [project])
  ...
  useEffect(() => {
    if (mode === 'edit' && selectedRound) {
      const fetchRound = async () => {
        try {
          const round = roundQuery.data?.find(round => round.id === selectedRound)
          if (round) {
            const datesToString = {
              ...round,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const submissionData = {
      ...
      projectSlug: projectSlug ?? projectSlugify(project.name),
