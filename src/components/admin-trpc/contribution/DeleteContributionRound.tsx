...
const DeleteContributionRound: FC<IDeleteContributionRoundProps> = ({ selectedProject }) => {
  ...
  const handleDeleteConfirm = () => {
    showDialog({
      ...
      description: `Are you sure you want to delete ${roundQuery.data?.find((item) => item.id === selectedRound)?.name}?`,
      buttons: [
  ...
  const handleDelete = async () => {
    try {
      await deleteRoundMutation.mutateAsync({ id: selectedRound! });
