...
const ContributeConfirm: FC<IContributeConfirmProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <Typography sx={{ mb: 2 }}>
          You are contributing {paymentAmount} {paymentCurrency} for{' '}
          {receiveAmount} {receiveCurrency}
        </Typography>
        <Typography sx={{ fontSize: '1rem!important', mb: 1 }}>
          You may choose an alternate wallet to contribute from, other than your
          signed in wallet.
        </Typography>
        <Button
          endIcon={
            <ExpandMoreIcon
              sx={{
                transform: openAlternativeWallet ? 'rotate(180deg)' : null,
              }}
            />
          }
          startIcon={
         ...
         <Collapse in={openAlternativeWallet}>
          {installedWallets.map((wallet) => (
            <WalletListItemComponent
              {...wallet}
              key={wallet.name}
              handleConnect={() => handleConnect(wallet.connectName)}
            />
          ))}
        </Collapse>
        {alternateWalletType && (
              ...
              >
                  ...
                  <Avatar
                    src={
                      theme.palette.mode === 'dark'
                        ? alternateWalletType?.iconDark
                        : alternateWalletType?.icon
                    }
                ...
                <Collapse in={errorMessage}>
                  <Box>
                    ...
                    >
                      Please choose a wallet with at least {paymentAmount} ADA
                      available
                    </Typography>
                  </Box>
                </Collapse>
