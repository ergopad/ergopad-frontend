    ...
    const getPageConfig = async () => {
      ...
      try {
        ...
        const config = res.data.filter(
          (config: { project: string | string[] | undefined }) =>
    ...
    const getStaked = async () => {
      ...
      try {
        ...
        if (wallets) {
          ...
          const url = `${process.env.API_URL}/staking/${project_id}/staked/`
          const res = await axios.post(url, request, { ...defaultOptions })
        ...
        if (address !== '') {
          ...
          const token = res.data.addresses[address].tokens.filter(
            (token: { tokenId: string }) =>
              token.tokenId === stakingConfig.tokenId
          )[0]
  ...
  const addStakeWithDappConnector = async () => {
    ...
    if (connected) {
      ...
      if (address === addstakeModalData.address) {
        ...
        ergoConnector.nautilus.disconnect()
        setErrorMessage(
          `Please connect the wallet with address ${addstakeModalData.address} `
        )
        setOpenError(true)
                  ...
                  <TabPanel value={tabValue} index={0}>
                    ...
                    >
                      ...
                      {stakingConfig.title === 'Neta' && (
                        ...
                        >
                          Neta staking currently disabled. You are still able to
                          unstake normally.
                        </Typography>
                      )}
                    </Box>
                    ...
                    <Paper sx={{ p: { xs: 2, sm: 4 }, borderRadius: 3 }}>
                      ...
                      {unstakeTableLoading ? (
                        ...
                        <UnstakingTable
                          ...
                          disableAddStake={stakingConfig.title === 'Neta'}
                        />
                      )}
                    </Paper>
                  </TabPanel>
                ...
                <>
                  ...
                  <Box>
                    <Typography color="text.secondary" sx={{ mb: 1 }}>
                      This address has {tokenBalance} {stakingConfig.title}{' '}
            ...
            >
              ...
              {ergopayUrl ? (
                ...
                <>
                  <Typography variant="body2" sx={{ fontSize: '1rem', mb: 2 }}>
                    You have{' '}
                    {getTokenBalanceLoading ? '...loading...' : tokenBalance}{' '}
                    {stakingConfig.title} tokens at this address. You can add
                    tokens to your existing stake box here.
                  </Typography>
                  <Box>
                    <Grid
                      container
