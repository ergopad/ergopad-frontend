...
const Settings: NextPage = () => {
  ...
  const [formErrors, setFormErrors] = useState<{ name: boolean, email: boolean }>({ name: false, email: false });
  const [formInput, setFormInput] = useState<{ name: string, email: string }>({ name: '', email: '' })
          ...
          ? (
            <Box sx={{ my: '30vh', textAlign: 'center' }}>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                <Typography>Return <Link href="/" passHref><MuiLink>Home</MuiLink></Link></Typography>
              </Box>
            </Box>
          )
          : (
